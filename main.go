...
func main() {
	...
	r.POST("/transaction", func(c *gin.Context) {
		...
		if qt.Type == "withdrawal" {
			...
			err := db.QueryRow("SELECT balance FROM users WHERE account_id = $1", qt.AccountID).Scan(&currentBalance)
			if err != nil {
...
func createUser(c *gin.Context, db *sql.DB) {
	...
	user.ID = fmt.Sprintf("user%d", len(user.Username)+len(user.Phone))
	...
	_, err = tx.Exec("INSERT INTO users (id, username, phone, account_id, balance) VALUES ($1, $2, $3, $4, 0)", user.ID, user.Username, user.Phone, user.AccountID)
	if err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
...
// validatePhone verifies a phone number matches the following format:
//
//  1. Optional plus sign
//  2. Between 1 and 14 digits
//
// This is a very basic validation, and may not catch all invalid phone numbers.
...
func processTransaction(c *gin.Context, db *sql.DB, transactionsCollection *mongo.Collection) {
	...
	var currentBalance float64
	err := db.QueryRow("SELECT balance FROM users WHERE account_id = $1", transaction.AccountID).Scan(&currentBalance)
	if err != nil {
		log.Printf("Database error querying balance: %v", err)
		if err == sql.ErrNoRows {
			c.JSON(http.StatusNotFound, gin.H{"error": "Account not found"})
	...
	result, err := db.Exec("UPDATE users SET balance = $1 WHERE account_id = $2", newBalance, transaction.AccountID)
	if err != nil {
...
func getUsers(c *gin.Context, db *sql.DB) {
	rows, err := db.Query("SELECT id, username, phone, account_id FROM users")
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	for rows.Next() {
		var u User
		if err := rows.Scan(&u.ID, &u.Username, &u.Phone, &u.AccountID); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
